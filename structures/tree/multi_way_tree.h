...
namespace structures
{
	...
	{
		if (son == nullptr) {
			throw std::invalid_argument("MultiWayTreeNode<T>::insertSon: son cannot be nullptr");
		}
		this->children_->insert(dynamic_cast<MultiWayTreeNode*>(son), order);
		son->setParent(this);
	}
	...
	{
		if (son == nullptr) {
			throw std::invalid_argument("MultiWayTreeNode<T>::replaceSon: son cannot be nullptr");
		}

		TreeNode<T>* oldSon = this->children_->at(order);
		oldSon->resetParent();
		this->children_->at(order) = dynamic_cast<MultiWayTreeNode*>(son);
		son->setParent(this);
		return oldSon;
	}
	...
	{
		TreeNode<T>* son = this->children_->removeAt(order);
		son->resetParent();
		return son;
	}
