...
namespace structures
{
	...
	inline void SequenceTable<K, T>::clear()
	{
		...
		list_->clear(); // z listu sa vyhadzu pointre
	}
	...
	inline SequenceTable<K, T>& SequenceTable<K, T>::assignSequenceTable(SequenceTable<K, T>& other)
	{
		if (this != &other) 
		{
			...
			{
				//this->insert(otherItem->getKey(), otherItem->accessData()); bolo by to neefektivne
				list_->add(new TableItem<K,T>(otherItem->getKey(), otherItem->accessData()));
			}
		}
		return *this;
	}
